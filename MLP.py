...
class NeuralNetwork:
    def __init__(self, input_nodes, hidden_layer, hidden_nodes, output_nodes):
        ...
        self.w.append(np.random.randn(self.input_nodes, self.hidden_nodes[0])/ np.sqrt(self.input_nodes/2))
        self.b.append(np.random.rand(self.hidden_nodes[0]))
        
        for i in range(hidden_layer-1):
          self.w.append(np.random.randn(self.hidden_nodes[i], self.hidden_nodes[i+1])/ np.sqrt(self.hidden_nodes[i]/2))
          self.b.append(np.random.rand(self.hidden_nodes[i+1]))

        self.w.append(np.random.rand(self.hidden_nodes[hidden_layer-1], output_nodes)/ np.sqrt(self.hidden_nodes[hidden_layer-1]/2))
        ...
        for i in range(hidden_layer):
          self.z.append(np.zeros([1,hidden_nodes[i]]))
          self.a.append(np.zeros([1,hidden_nodes[i]]))
    ...
    def train(self, input_data, target_data):  
        ...
        for i in range(0, (self.hidden_layer+1)*-1 , -1):
          if i == 0:
            loss = (self.a[-1]-self.target_data) * self.a[-1] * (1-self.a[-1])
            self.w[i-1] = self.w[i-1] - self.learning_rate * np.dot(self.a[i-2].T, loss)
            self.b[i-1] = self.b[i-1] - self.learning_rate * loss
          else:
            loss = np.dot(loss, self.w[i].T) * self.a[i-1] * (1-self.a[i-1])
            self.w[i-1] = self.w[i-1] - self.learning_rate * np.dot(self.a[i-2].T, loss)
            self.b[i-1] = self.b[i-1] - self.learning_rate * loss
    ...
    def predict(self, input_data):
      ...
      for i in range(self.hidden_layer+1):
        if i == 0:
          self.z[i+1] = np.dot(input_data, self.w[i]) + self.b[i]
          self.a[i+1] = sigmoid(self.z[i+1])
        else:
          self.z[i+1] = np.dot(self.a[i], self.w[i]) + self.b[i]
          self.a[i+1] = sigmoid(self.z[i+1])
